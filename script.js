let currentTrackUrl = ""; // will hold the active song's full URL
let songs;
let currfolder;
...
const playmusic = (track, pause = false) => {
    ...
    if (!currentsong) {
        console.error("⛔ Audio player element (currentsong) is missing");
        return;
    }
    ...
    console.log("🎵 Playing track from:", songUrl);
    ...
    const cleanName = decodeURIComponent(track.split("/").pop());
...
async function displayalbum() {
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (
            ...
            albumfolder = albumfolder.toLowerCase().replaceAll(" ", "_");
